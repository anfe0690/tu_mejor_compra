...
public class Navegacion implements Serializable {
    ...
    public List<Producto> calcular(TypedQuery<Producto> tq) {
        ...
        if (pagina < 1 || pagina > numeroPaginas) {
            ...
            if (numeroPaginas == 0) {
                String s = "Ningun producto fue encontrado.";
                logger.info(s);
                facesContext.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, s, null));
            } else {
                String s = "Numero de pagina \"" + pagina + "\" invalido.";
                logger.warn(s);
                facesContext.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, s, null));
            }
