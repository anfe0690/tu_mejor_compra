...
//            }
    ...
    private void crearImagen(Usuario usuario, Producto producto) {
        ...
        String dirDestinoBase = "C:\\var\\tuMejorCompra\\img\\" + usuario.getNombre() + "\\";
    ...
    public String restaurarTodosLosDatos() {
        ...
        for (Usuario u : q.getResultList()) {
            miLogger.log(u.toString());

            List<Venta> vs = u.getVentas();
            u.setVentas(null);
            ...
            for (Producto p : u.getProductos()) {
                File f = new File("C:\\var\\tuMejorCompra\\img\\" + u.getNombre() + "\\" + p.getNombreImagen());
                try {
        ...
        miLogger.log("La base de datos fue restaurada.");
        return "index?faces-redirect=true";
    }
